...
while (true) {
  ...
  if (entrada.length > Math.pow(10, 4)) continue;

  const sentence = entrada.trim().match(regexPattern)
  ...
  const abreviacao = `${word[0]}.`
  const legend = `${abreviacao} = ${word}`
  const regexp = new RegExp(`\\b${word}\\b`, 'g')
  return { word, repeatations, trim, save, abreviacao, legend, regexp }
  }).filter(word => word.save > 0)

  const abreviados = alfabeto.split('').map(letter => {
  ...
  nova = entrada
  for (const abc of abreviados) {
    nova = nova.replace(abc.regexp, abc.abreviacao)
  }

  console.log(nova);
  console.log(abreviados.length);
  abreviados.sort().map(({ legend }) => console.log(legend))
}
